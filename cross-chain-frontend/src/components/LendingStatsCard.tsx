  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        console.log("Data from getActiveLoans:", all);

        const allLoans = [...completed, ...all];
        // const volume = allLoans.reduce(
        //   (sum, loan) => sum + Number(loan.loanAmount),
        //   0
        // );
        const volume = await getTotalLoanVolume();
        ...
        const active = all.filter(
          (loan) => !loan.completed && loan.active && !loan.cancelled
        ).length;
        console.log("Active Loans:", active);
        const completedLoans = all.filter((loan) => loan.completed).length;
        const cancelledLoans = all.filter((loan) => loan.cancelled).length;

        setTotalLoans(all.length);
