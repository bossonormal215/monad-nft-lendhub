...
const NFTCard = ({ nft, onSelect, selected }: NFTCardProps) => {
  return (
    ...
    >
      {nft.image && (
        <img src="../app/favicon.ico" alt={nft.name} className="w-full h-48 object-cover rounded-lg mb-4" />
      )}
      <h3 className="font-bold text-lg text-white">{nft.name}</h3>
      {nft.description && <p className="text-sm text-gray-400 mt-2">{nft.description}</p>}
      ...
      {selectedNFT && (
        <div className="mt-8 p-4 border rounded-lg bg-gray-800 border-gray-700">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1 text-white">Token ID</label>
              <input
                type="number"
                value={tokenId}
                onChange={(e) => {
